...
| Outbox relay | Default `OUTBOX_SINGLE_ACTIVE=true` = one active relay per service (advisory-lock leader); extra replicas are warm standbys (takeover ≤ ~2× poll interval). **For ordered parallel publish, set `OUTBOX_RELAY_SHARDS=N`** (ADR-0017): N independently leader-elected shard relays, each owning the aggregates that hash into it — ~N× throughput while per-aggregate order is preserved | First tune vertically: raise `OUTBOX_BATCH_SIZE`, lower `OUTBOX_POLL_INTERVAL`. Then shard (`OUTBOX_RELAY_SHARDS`, N ≤ instance count to actually parallelise) — this is the Tier-2 publish-throughput lever. Last resort `OUTBOX_SINGLE_ACTIVE=false` trades per-aggregate ordering for unordered parallel publish — only with reorder-safe consumers |
