...
| `audit` | `platform/security/audit` | `Behavior` (`audit.Audit`) — on successful command writes an audit entry (who/what/when/resource) inside the command transaction via `pg.FromContext`; records the originating tenant in the (hash-chained) metadata. `BufferedAuditWriter` + `AsyncAudit[C,R]` are the Eventual-consistency pair: `AsyncAudit` enqueues the same entry after handler success (never fails the command); `BufferedAuditWriter` drains the buffer in the background via `RecordBatchSameChain` and emits `audit.dropped_total` / `audit.async_write_errors_total` metrics. Buffered entries are lost on crash — best-effort by design; tamper-evidence (`VerifyChain`) still holds over flushed entries. Effectively-once delivery is never relaxed — only audit completeness and latency trade-off |
