...
| `telemetry` | `platform/observability/telemetry` | OTel tracer + meter providers (incl. Go runtime metrics) with OTLP/gRPC + Prometheus exporters (`/metrics` reader stays on even when export is disabled); opt-in OTLP logger provider (`TELEMETRY_LOGS`, bridged into `log` for stdout+collector fan-out); `Shutdown` |
...
| `httpx` | `platform/web/httpx` | `Decode`+validate request bodies; RFC 9457 problem+json responses with `code`/`params`/`instance` extension members; `FromError` maps `apperr` errors; `ProblemLocalizer` ctx seam for i18n |
| `apperr` | `platform/apperr` | Application error model: flat UPPER_SNAKE codes + HTTP status + permanence + params; init-time registry (duplicate panics); `IsPermanent` drives the DLT short-circuit; registry generates `docs/errors.md` (cmd/errgen) |
| `i18n` | `platform/i18n` | go-i18n v2 bundle over embedded TOML catalogs; `Accept-Language` negotiation middleware; localizes problem title/detail via the httpx `ProblemLocalizer` seam — `code`/`params` stay locale-independent |
| `clock` | `platform/clock` | `Clock` interface + `System` (UTC `time.Now`); injected only where business logic reads "now"; DB row timestamps stay `DEFAULT now()` |
...
| `cqrs` | `platform/cqrs` | `HandlerFunc[C,R]`, `Behavior[C,R]`, `Decorate`, `StandardPipeline`, `Deadline` — typed generic decorator pipeline; `ConsistencyPolicy` (`Strong`/`Eventual` presets, four bool axes: `Transactional`/`SyncAudit`/`SyncRYW`/`SyncProjection`) declares per-command-type consistency; `Pipeline.WithConsistency(policy, pool)` gates the Transaction behavior on `policy.Transactional`; `Pipeline.WithOutbox()` + `Transactional:false` panics at startup (effectively-once guard); Strong is the default — relaxation is explicit, never implicit |
...
| `audit` | `platform/security/audit` | Three consistency modes share the same hash-chained `audit_entries` table and `VerifyChain` tamper-evidence guarantee. **Strong** (`audit.Audit`) writes the entry inside the command transaction (audit-at-commit; chain-head lock on hot path). **Eventual** (`audit.AsyncAudit` + `BufferedAuditWriter`) enqueues after handler success and drains in the background via `RecordBatchSameChain`; best-effort — entries are lost on crash or buffer overflow, metered via `audit.dropped_total` / `audit.async_write_errors_total`. **Durable** (`audit.DurableAudit`) stages a lightweight intent row into `audit_pending` inside the command transaction (durable and effectively-once; no chain-head lock on the hot path); a single-active per-shard drain worker (`servicekit.AddAuditDrain`) reads `audit_pending` ordered by `(chain_id, id)` and applies intents to the hash chain via `DrainPending` — never drops, tamper-evidence preserved across the async hop, drain lag metered via `audit.pending_backlog`. Tenant propagation is automatic across all three modes (ADR-0015). See `docs/superpowers/specs/2026-06-13-durable-audit-design.md` for the Durable design |
...
| `servicekit` | `platform/servicekit` | Service wiring harness: `New` (+`WithoutKafka`/`WithoutPG`), `AddConsumer`/`AddConsumerWithRetry`/`AddOutboxRelay`/`AddWorker`/`AddHTTPServer`/`AddAuditCleanup`/`AddAuditDrain`, `Main` entry point; readiness-first drain-gate teardown. `AddAuditDrain(interval, batchSize, ...audit.Option)` registers one single-active (leader-elected) drain goroutine per physical audit shard — required for `audit.DurableAudit` |
...
Error contract: every error a request can surface carries a registered `platform/apperr` code. At the HTTP edge `httpx.FromError` renders it as RFC 9457 problem+json whose machine-readable members are `code` (flat UPPER_SNAKE, service-prefixed) and `params` (every message variable, per AIP-193) — `title`/`detail` are human-readable, localized per `Accept-Language` when the i18n middleware ran, and never part of the contract. Unknown errors collapse to `500 INTERNAL` without leaking internals. On the async side the same model drives retry semantics: a permanent apperr short-circuits `kafka.WithRetry` and the tiered-retry escalator straight to the DLT with the code in the `x-error-code` header. The full catalog is generated from the live registry into [`docs/errors.md`](errors.md); process and rules in [`docs/conventions.md`](conventions.md) §10.
...
| Table partitioning / age-based cleanup | Age-based cleanup (polling delete of old published outbox rows, old audit rows) is wired. **Outbox** is additionally RANGE-partitioned by `created_at` from day one with an opt-in maintenance worker (`OUTBOX_PARTITION_MODE=partitioned`, ADR-0016): default "simple" mode uses the DEFAULT partition + age-based DELETE with zero overhead; partitioned mode DETACH+DROPs expired partitions (never one holding unpublished rows). **Audit is deliberately NOT partitioned** — DROPping the oldest partition would delete the hash-chain genesis row, which `VerifyChain` is built to flag as tampering (ADR-0016 § "Why audit is NOT partitioned"). |
| Read replica pool routing | WIRED: `pg.Pool` splits reader/writer (`PG_READER_DSN` + independent `PG_READER_MAX_CONNS`/`PG_READER_MIN_CONNS` sizing; `Reader()` falls back to the writer pool when no replica is configured). The gateway's query path routes through `pg.FromContextRead` (GetOrder, ListOrders, attachment ownership lookup, SSE last-status, audit DSAR/VerifyChain), so reads hit the replica while commands and read-your-writes stay on the writer. Command-side repos (orders/payments) read via `pg.FromContext` because they run inside the command transaction. |
