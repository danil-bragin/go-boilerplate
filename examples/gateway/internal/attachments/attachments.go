...
// Auditor records a security-event audit entry out-of-band (separate tx).
// Implemented by *audit.PgStore.RecordOutOfBand via auditFunc; kept as a small
// interface so the handler does not depend on the audit package directly.
type Auditor interface {
	RecordOutOfBand(ctx context.Context, e audit.Entry) error
}
...
// WithAuditor enables best-effort audit entries on attachment access (upload /
// download success) and on access denials (authz/ownership 403). nil leaves
// auditing off (boilerplate demo mode).
...
// auditAccess records a successful attachment access out-of-band (best-effort).
func (h *Handler) auditAccess(ctx context.Context, p auth.Principal, action, key string) {
	if h.auditor == nil {
		return
	}
	_ = h.auditor.RecordOutOfBand(ctx, audit.Entry{
		Actor:    p.Subject,
		Action:   action,
		Subject:  key,
...
// auditDenied records an attachment access DENIAL out-of-band (best-effort).
// Only a PRESENT principal is audited (the caller passes the principal it
// already resolved) — anonymous 401s are never recorded.
func (h *Handler) auditDenied(ctx context.Context, p auth.Principal, orderID string) {
	if h.auditor == nil {
		return
	}
	_ = h.auditor.RecordOutOfBand(ctx, audit.Entry{
		...
		Subject:  orderID,
		Metadata: map[string]string{"denied_action": "attachment:access"},
	})
}
...
func (h *Handler) roleGate(w http.ResponseWriter, r *http.Request) (auth.Principal, bool) {
	...
	if !hasRole(p, h.requiredRole) && !hasRole(p, AdminRole) {
		h.auditDenied(r.Context(), p, chi.URLParam(r, "id"))
...
func (h *Handler) Upload(w http.ResponseWriter, r *http.Request) {
	...
	h.auditAccess(ctx, p, audit.ActionAttachmentUpload, key)
...
func (h *Handler) Download(w http.ResponseWriter, r *http.Request) {
	...
	h.auditAccess(ctx, p, audit.ActionAttachmentDownload, key)
