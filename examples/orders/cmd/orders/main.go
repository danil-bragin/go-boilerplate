// Command orders is the orders service: consumes CreateOrderCommand from Kafka,
// writes orders to Postgres, and emits OrderCreated events via the transactional outbox.
package main
...
// appConfig aggregates all configuration for the orders service.
...
// ordersApp holds all wired components.
...
func newApp(ctx context.Context) (*ordersApp, error) {
	...
	commandHandler := transport.NewCommandHandler(pool, decoratedHandler)
...
func (a *ordersApp) stop(ctx context.Context) error {
